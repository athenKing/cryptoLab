...
{	
	...
	std::vector<string> fmSketch = load_Data_into_FM(batchNum,numOTs,"../data/bob.data");

	cout<<"finished generate bob fmsketch"<<endl;
    ...
    timing_end(&tm);
    printf("\nCompleted Flajolet-Martin transfer in %f seconds!\n", timing_get_difference(tm));


    timing_end(&tm0);
    printf("\nCompleted total in %f seconds!\n", timing_get_difference(tm0));
